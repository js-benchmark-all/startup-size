...
let running = false;
const run = async (runtime: typeof config.RUNTIMES[keyof typeof config.RUNTIMES]) => {
      ...
      results[categoryName] = {
        ...
        datasets: [
          ...
          ...[50, 75, 99, 999].map((p) => {
            ...
            return {
              label,
              data: categoryResults.map((v) => utils.nsToMs(utils.percentile(v.values, p)))
            }
          })
        ]
      };
...
const RUNTIME = config.RUNTIMES[process.argv[2] as keyof typeof config.RUNTIMES];
if (RUNTIME != null)
  await run(RUNTIME);
else
